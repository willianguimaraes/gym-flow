<template>
  <div class="week-strip">
    <div
      v-for="(day, idx) in diasSemana"
      :key="idx"
      class="day-pill"
      :class="{ active: selectedDay === idx, rest: day.isRest }"
...
export default {
    ...
    setup() {
        ...
        const diasSemana = ref<DiasTreino[]>([]);
...
<style scoped>
  ...
  .day-pill.active .day-abbr { color: rgba(255,255,255,0.8); }
  .day-num {
    font-size: 18px;
    font-weight: 600;
    color: #fff;
  ...
  .day-pill.active .day-dot { background: rgba(255,255,255,0.6); }
  .day-pill.rest .day-dot { background: rgba(255,255,255,0.2); }
</style>